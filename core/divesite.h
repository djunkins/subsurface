...
static inline struct dive_site *get_dive_site(int nr, struct dive_site_table *table)
{
	if (nr >= table->nr || nr < 0)
		return NULL;
	return table->dive_sites[nr];
}

/* iterate over each dive site */
#define for_each_dive_site(_i, _x, _table) \
	for ((_i) = 0; ((_x) = get_dive_site(_i, _table)) != NULL; (_i)++)
...
struct dive_site *create_dive_site_with_gps(const char *name, const location_t *, timestamp_t divetime, struct dive_site_table *table);
